 ...
 */
public abstract class FacebookDialogBase<CONTENT, RESULT> implements
	...
	@Override
	public final void registerCallback(final CallbackManager callbackManager,
	...
	@Override
	public final void registerCallback(final CallbackManager callbackManager,
			final FacebookCallback<RESULT> callback, final int requestCode) {
	...
	protected boolean canShowImpl(CONTENT content, Object mode) {
		...
		for (ModeHandler handler : cachedModeHandlers()) {
			if (!anyModeAllowed
					&& !Utility.areObjectsEqual(handler.getMode(), mode)) {
				continue;
			}
	...
	protected void showImpl(final CONTENT content, final Object mode) {
		...
		if (appCall != null) {
			...
			// If we got a null appCall, then the derived dialog code is doing
			// something wrong
	...
	private AppCall createAppCallForMode(final CONTENT content,
			final Object mode) {
		boolean anyModeAllowed = (mode == BASE_AUTOMATIC_MODE);
