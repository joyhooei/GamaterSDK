 ...
 * {@link #setAppId(String)} in their application startup path. For specifying
 * in the manifest, add a meta tag in your &lt;application&gt; tag.
 *
 * <pre>
 ...
 *
 * <p>
 * When sharing to Messenger, apps should call the {@link #shareToMessenger}
 * method. For example,
 *
 * <pre>
 * ShareToMessengerParams params = ShareToMessengerParams
 * 		.newBuilder(uri, &quot;image/*&quot;).setMetaData(metaData).build();
 * MessengerUtils.shareToMessenger(this, REQUEST_CODE_SHARE_TO_MESSENGER, params);
 * </pre>
 *
 * To handle receiving a composer shortcut or reply intent from Messenger, apps
 * should put the following intent filter in their manifest for the activity
 ...
 * </pre>
 *
 * When handling the intent, then call
 * {@link #getMessengerThreadParamsForIntent} to receive the parameters for
 * messenger. When the user has clicked the Sened button to send the content to
 * Messenger, ten call {@link #finishShareToMessenger} to return the data back
 * to Messenger.
 * </p>
 */
public class MessengerUtils {
	...
	/**
	 * Starts an intent to share a piece of media on Messenger using the
	 * messenger content platform.
	 *
	 * @param activity
	 *            the activity sharing the content
	 * @param requestCode
	 *            a unique request code for
	 *            {@link Activity#startActivityForResult}
	 * @param shareToMessengerParams
	 *            parameters for what to share
	 */
	public static void shareToMessenger(Activity activity, int requestCode,
		...
		if (allAvailableVersions.contains(PROTOCOL_VERSION_20150314)) {
			shareToMessenger20150314(activity, requestCode,
	...
	private static void shareToMessenger20150314(Activity activity,
			int requestCode, ShareToMessengerParams shareToMessengerParams) {
		try {
			Intent shareIntent = new Intent(Intent.ACTION_SEND);
			shareIntent.setFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
			shareIntent.setPackage(PACKAGE_NAME);
			shareIntent.putExtra(Intent.EXTRA_STREAM,
					shareToMessengerParams.uri);
			shareIntent.setType(shareToMessengerParams.mimeType);
			String appId = FacebookSdk.getApplicationId();
			if (appId != null) {
				shareIntent.putExtra(EXTRA_PROTOCOL_VERSION,
						PROTOCOL_VERSION_20150314);
				shareIntent.putExtra(EXTRA_APP_ID, appId);
				shareIntent.putExtra(EXTRA_METADATA,
						shareToMessengerParams.metaData);
				shareIntent.putExtra(EXTRA_EXTERNAL_URI,
						shareToMessengerParams.externalUri);
			}

			activity.startActivityForResult(shareIntent, requestCode);
		} catch (ActivityNotFoundException e) {
			Intent openMessenger = activity.getPackageManager()
	...
	/**
	 * When handling an {@code Intent} from Messenger, call this to parse the
	 * parameters of the intent.
	 *
	 * @param intent
	 *            the intent of the activity
	 * @return a {@link MessengerThreadParams} or null if this intent wasn't
	 *         recognized as a request from Messenger to share.
	 */
	public static MessengerThreadParams getMessengerThreadParamsForIntent(
			Intent intent) {
		...
		if (categories.contains(ORCA_THREAD_CATEGORY_20150314)) {
			...
			String threadToken = appLinkExtras
					.getString(EXTRA_THREAD_TOKEN_KEY);
			...
			return new MessengerThreadParams(origin, threadToken, metadata,
	...
	/**
	 * Finishes the activity and returns the media item the user picked to
	 * Messenger.
	 *
	 * @param activity
	 *            the activity that received the original intent from Messenger
	 * @param shareToMessengerParams
	 *            parameters for what to share
	 */
	public static void finishShareToMessenger(Activity activity,
		...
		if (categories.contains(ORCA_THREAD_CATEGORY_20150314)) {
			...
			if (categories.contains(ORCA_THREAD_CATEGORY_20150314)) {
				resultIntent.putExtra(EXTRA_PROTOCOL_VERSION,
						MessengerUtils.PROTOCOL_VERSION_20150314);
				String threadToken = appLinkExtras
						.getString(MessengerUtils.EXTRA_THREAD_TOKEN_KEY);
				resultIntent.putExtra(EXTRA_THREAD_TOKEN_KEY, threadToken);
			} else {
				throw new RuntimeException(); // Can't happen.
			}
			resultIntent.setDataAndType(shareToMessengerParams.uri,
					shareToMessengerParams.mimeType);
			resultIntent.setFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
			resultIntent.putExtra(EXTRA_APP_ID, FacebookSdk.getApplicationId());
			resultIntent.putExtra(EXTRA_METADATA,
					shareToMessengerParams.metaData);
			resultIntent.putExtra(EXTRA_EXTERNAL_URI,
					shareToMessengerParams.externalUri);
	...
	private static Set<Integer> getAllAvailableProtocolVersions(Context context) {
		...
		Uri uri = Uri
				.parse("content://com.facebook.orca.provider.MessengerPlatformProvider/versions");
