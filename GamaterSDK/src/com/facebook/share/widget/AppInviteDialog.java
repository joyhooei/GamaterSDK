 ...
 */
public class AppInviteDialog extends
		FacebookDialogBase<AppInviteContent, AppInviteDialog.Result> {
	...
	private static final int DEFAULT_REQUEST_CODE = CallbackManagerImpl.RequestCodeOffset.AppInvite
			.toRequestCode();
	 ...
	 * {@link com.facebook.share.model.AppInviteContent} using the provided
	 * Activity. No callback will be invoked.
	 *
	 * @param activity
	 *            Activity to use to share the provided content
	 * @param appInviteContent
	 *            Content of the app invite to send
	 */
	 ...
	 *
	 * @param fragment
	 *            Fragment to use to share the provided content
	 * @param appInviteContent
	 *            Content of the app invite to send
	 */
	...
	private static boolean canShowNativeDialog() {
		return (Build.VERSION.SDK_INT >= ShareConstants.MIN_API_VERSION_FOR_WEB_FALLBACK_DIALOGS)
				&& DialogPresenter
						.canPresentNativeDialogWithFeature(getFeature());
	}

	private static boolean canShowWebFallback() {
		return (Build.VERSION.SDK_INT >= ShareConstants.MIN_API_VERSION_FOR_WEB_FALLBACK_DIALOGS)
				&& DialogPresenter
						.canPresentWebFallbackDialogWithFeature(getFeature());
	}
	...
	protected void registerCallbackImpl(
		...
		final ResultProcessor resultProcessor = (callback == null) ? null
				: new ResultProcessor(callback) {
					@Override
					public void onSuccess(AppCall appCall, Bundle results) {
						String gesture = ShareInternalUtility
								.getNativeDialogCompletionGesture(results);
		...
		callbackManager.registerCallback(getRequestCode(),
	...
	private class NativeHandler extends ModeHandler {
		...
		public AppCall createAppCall(final AppInviteContent content) {
			...
			DialogPresenter.setupAppCallForNativeDialog(appCall,
					new DialogPresenter.ParameterProvider() {
						...
						public Bundle getLegacyParameters() {
							// App Invites are not supported with legacy fb4a
							// devices.
	...
	private class WebFallbackHandler extends ModeHandler {
		...
		public AppCall createAppCall(final AppInviteContent content) {
			...
			DialogPresenter.setupAppCallForWebFallbackDialog(appCall,
					createParameters(content), getFeature());
	...
	private static Bundle createParameters(final AppInviteContent content) {
		...
		params.putString(ShareConstants.PREVIEW_IMAGE_URL,
				content.getPreviewImageUrl());
