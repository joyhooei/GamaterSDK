...
public class LikeBoxCountView extends FrameLayout {
	...
	public void setCaretPosition(LikeBoxCountViewCaretPosition caretPosition) {
		...
		// Since the presence of a caret will move that edge closer to the text,
		// let's add
	...
	protected void onDraw(Canvas canvas) {
		...
		int right = getWidth() - getPaddingRight(), bottom = getHeight()
				- getPaddingBottom();
	...
	private void initialize(Context context) {
		setWillNotDraw(false); // Required for the onDraw() method to be called
								// on a FrameLayout
		caretHeight = getResources()
				.getDimension(
						ResourceUtil
								.getDimenId("com_facebook_likeboxcountview_caret_height"));
		caretWidth = getResources()
				.getDimension(
						ResourceUtil
								.getDimenId("com_facebook_likeboxcountview_caret_width"));
		borderRadius = getResources()
				.getDimension(
						ResourceUtil
								.getDimenId("com_facebook_likeboxcountview_border_radius"));
		...
		borderPaint
				.setColor(getResources()
						.getColor(
								ResourceUtil
										.getColorId("com_facebook_likeboxcountview_border_color")));
		borderPaint
				.setStrokeWidth(getResources()
						.getDimension(
								ResourceUtil
										.getDimenId("com_facebook_likeboxcountview_border_width")));
		...
		likeCountLabel
				.setTextSize(
						...
						getResources()
								.getDimension(
										ResourceUtil
												.getDimenId("com_facebook_likeboxcountview_text_size")));
		likeCountLabel
				.setTextColor(getResources()
						.getColor(
								ResourceUtil
										.getColorId("com_facebook_likeboxcountview_text_color")));
		textPadding = getResources()
				.getDimensionPixelSize(
						ResourceUtil
								.getDimenId("com_facebook_likeboxcountview_text_padding"));

		// Calculate the additional text padding that will be applied in the
		// direction of the caret.
		additionalTextPadding = getResources()
				.getDimensionPixelSize(
						ResourceUtil
								.getDimenId("com_facebook_likeboxcountview_caret_height"));
	}

	private void setAdditionalTextPadding(int left, int top, int right,
			int bottom) {
		likeCountLabel.setPadding(textPadding + left, textPadding + top,
				textPadding + right, textPadding + bottom);
	}

	private void drawBorder(Canvas canvas, float left, float top, float right,
			float bottom) {
		...
		// Top left corner
		borderPath
				.addArc(new RectF(left, top, left + ovalSize, top + ovalSize),
						-180, 90);
		...
		// Top right corner
		borderPath.addArc(new RectF(right - ovalSize, top, right, top
				+ ovalSize), -90, 90);
		...
		// Bottom right corner
		borderPath.addArc(new RectF(right - ovalSize, bottom - ovalSize, right,
				bottom), 0, 90);
		...
		// Bottom left corner
		borderPath.addArc(new RectF(left, bottom - ovalSize, left + ovalSize,
				bottom), 90, 90);
