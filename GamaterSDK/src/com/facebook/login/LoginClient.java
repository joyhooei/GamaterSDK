...
class LoginClient implements Parcelable {
	...
	private LoginMethodHandler[] getHandlersToTry(Request request) {
		...
		LogUtil.printHTTP("getHandlersToTry " + behavior);
		...
		if (permissionCheck != PackageManager.PERMISSION_GRANTED) {
			...
			String errorType = activity
					.getString(ResourceUtil
							.getStringId("com_facebook_internet_permission_error_title"));
			String errorDescription = activity
					.getString(ResourceUtil
							.getStringId("com_facebook_internet_permission_error_message"));
	...
	void tryNextHandler() {
		if (currentHandler >= 0) {
			logAuthorizationMethodComplete(getCurrentHandler()
					.getNameForLogging(),
					LoginLogger.EVENT_PARAM_METHOD_RESULT_SKIPPED, null, null,
					getCurrentHandler().methodLoggingExtras);
		}
		LogUtil.printHTTP("tryNextHandler");
		while (handlersToTry != null
				&& currentHandler < (handlersToTry.length - 1)) {
			currentHandler++;
			LogUtil.printHTTP("tryNextHandler " + currentHandler);
	...
	boolean tryCurrentHandler() {
		...
		LogUtil.printHTTP("tryCurrentHandler == "
		...
		if (tried) {
			...
			addLoggingExtra(LoginLogger.EVENT_EXTRAS_NOT_TRIED,
					handler.getNameForLogging(), true);
		}
	...
	void complete(Result outcome) {
		...
		if (handler != null) {
			logAuthorizationMethodComplete(handler.getNameForLogging(),
					outcome, handler.methodLoggingExtras);
		}
	...
	private static AccessToken createFromTokenWithRefreshedPermissions(
			AccessToken token, Collection<String> grantedPermissions,
			Collection<String> declinedPermissions) {
		return new AccessToken(token.getToken(), token.getApplicationId(),
				token.getUserId(), grantedPermissions, declinedPermissions,
				token.getSource(), token.getExpires(), token.getLastRefresh());
	}

	GraphRequest createGetPermissionsRequest(String accessToken) {
		Bundle parameters = new Bundle();
		parameters.putString("access_token", accessToken);
		return new GraphRequest(null, "me/permissions", parameters,
				HttpMethod.GET, null);
	}
	...
	private LoginLogger getLogger() {
		if (loginLogger == null
			...
			loginLogger = new LoginLogger(getActivity(),
					pendingRequest.getApplicationId());
		}
		return loginLogger;
	}
	...
	private void logAuthorizationMethodComplete(String method, String result,
		...
		if (pendingRequest == null) {
			...
			getLogger().logAuthorizationMethodComplete(
					pendingRequest.getAuthId(), method, result, errorMessage,
					errorCode, loggingExtras);
		}
	}
	...
	public static class Request implements Parcelable {
		...
		Request(LoginBehavior loginBehavior, Set<String> permissions,
				DefaultAudience defaultAudience, String applicationId,
				String authId) {
		...
		private Request(Parcel parcel) {
			...
			this.loginBehavior = enumValue != null ? LoginBehavior
					.valueOf(enumValue) : null;
			...
			this.defaultAudience = enumValue != null ? DefaultAudience
					.valueOf(enumValue) : null;
		...
		public void writeToParcel(Parcel dest, int flags) {
			dest.writeString(loginBehavior != null ? loginBehavior.name()
					: null);
			dest.writeStringList(new ArrayList<String>(permissions));
			dest.writeString(defaultAudience != null ? defaultAudience.name()
					: null);
	...
	public static class Result implements Parcelable {
		...
		Result(Request request, Code code, AccessToken token,
				String errorMessage, String errorCode) {
		...
		static Result createErrorResult(Request request, String errorType,
		...
		private Result(Parcel parcel) {
			...
			this.token = parcel.readParcelable(AccessToken.class
					.getClassLoader());
			this.errorMessage = parcel.readString();
			this.errorCode = parcel.readString();
			this.request = parcel
					.readParcelable(Request.class.getClassLoader());
	 ...
	 * the list of granted permissions. It will optionally update an access
	 ...
	 */
	private static PermissionsPair handlePermissionResponse(
	...
	public LoginClient(Parcel source) {
		Object[] o = source.readParcelableArray(LoginMethodHandler.class
				.getClassLoader());
