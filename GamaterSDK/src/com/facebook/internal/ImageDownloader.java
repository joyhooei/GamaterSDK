...
public class ImageDownloader {
	...
	public static void downloadAsync(ImageRequest request) {
		...
		// book-keeping with the
		// requests object reference. So we keep the old references and just map
		// them to new urls in
		// the downloader.
	...
	public static boolean cancelRequest(ImageRequest request) {
		...
		synchronized (pendingRequests) {
			...
			if (downloaderContext != null) {
				...
				// issued. This is
				// regardless of whether a cache-read or network-download is
				// underway for this
				// request.
				cancelled = true;
	...
	private static void enqueueRequest(ImageRequest request, RequestKey key,
		...
		synchronized (pendingRequests) {
			...
			// request has been
			// registered. This is necessary since the WorkItem might kick off
			// right away and
			// attempt to retrieve the request's DownloaderContext prior to it
			// being ready for
			// access.
			//
			// It is also necessary to hold on to the lock until after the
			// workItem is created,
			// since calls to cancelRequest or prioritizeRequest might come in
			// and expect a
		...
		try {
			...
			default:
				...
				if (stream != null) {
					...
					errorMessageBuilder
							.append(context.getString(ResourceUtil
									.getStringId("com_facebook_image_download_unknown_error")));
				}
