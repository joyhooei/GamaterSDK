...
public class LikeActionController {
	...
	private static final String TAG = LikeActionController.class
			.getSimpleName();

	private static final int LIKE_ACTION_CONTROLLER_VERSION = 3;
	private static final int MAX_CACHE_SIZE = 128;
	// MAX_OBJECT_SUFFIX basically accommodates for 1000 access token changes
	// before the async
	...
	/**
	 * Called from CallbackManager to process any pending likes that had
	 * resulted in the Like dialog being displayed
	 *
	 * @param requestCode
	...
	public static boolean handleOnActivityResult(final int requestCode,
		...
		if (Utility.isNullOrEmpty(objectIdForPendingController)) {
			...
			SharedPreferences sharedPreferences = appContext
					.getSharedPreferences(LIKE_ACTION_CONTROLLER_STORE,
		...
		getControllerForObjectId(objectIdForPendingController,
				LikeView.ObjectType.UNKNOWN, new CreationCallback() {
					@Override
					public void onComplete(
							LikeActionController likeActionController,
							FacebookException error) {
						if (error == null) {
							likeActionController.onActivityResult(requestCode,
									resultCode, data);
						} else {
	 ...
	 *            Object Id
	 * @param callback
	 *            Callback to be invoked when the LikeActionController has been
	 *            created.
	 */
	public static void getControllerForObjectId(String objectId,
			LikeView.ObjectType objectType, CreationCallback callback) {
		if (!isInitialized) {
		...
		if (controllerForObject != null) {
			...
			verifyControllerAndInvokeCallback(controllerForObject, objectType,
					callback);
		} else {
			diskIOWorkQueue
					.addActiveWorkItem(new CreateLikeActionControllerWorkItem(
							objectId, objectType, callback));
		}
	}
	...
	private static void verifyControllerAndInvokeCallback(
			LikeActionController likeActionController,
			LikeView.ObjectType objectType, CreationCallback callback) {
		LikeView.ObjectType bestObjectType = ShareInternalUtility
				.getMostSpecificObjectType(objectType,
						likeActionController.objectType);
		FacebookException error = null;
		if (bestObjectType == null) {
			// Looks like the existing controller has an object_type for this
			// object_id that is
	...
	/**
	 * NOTE: This MUST be called ONLY via the CreateLikeActionControllerWorkItem
	 * class to ensure that it happens on the right thread, at the right time.
	 */
	private static void createControllerForObjectIdAndType(String objectId,
			LikeView.ObjectType objectType, CreationCallback callback) {
		// Check again to see if the controller was created before attempting to
		// deserialize/create
		// one. Need to check this in the case where multiple LikeViews are
		// looking for a controller
		// for the same object and all got queued up to create one. We only want
		// the first one to go
		// through with the creation, and the rest should get the same instance
		// from the
		// object-cache.
		LikeActionController controllerForObject = getControllerFromInMemoryCache(objectId);
		if (controllerForObject != null) {
	...
	private synchronized static void performFirstInitialize() {
		...
		SharedPreferences sharedPreferences = appContext.getSharedPreferences(
				LIKE_ACTION_CONTROLLER_STORE, Context.MODE_PRIVATE);

		objectSuffix = sharedPreferences.getInt(
				LIKE_ACTION_CONTROLLER_STORE_OBJECT_SUFFIX_KEY, 1);
	...
	private static void invokeCallbackWithController(
			...
			final LikeActionController controller, final FacebookException error) {
		if (callback == null) {
			return;
		}
		...
		accessTokenTracker = new AccessTokenTracker() {
			...
			protected void onCurrentAccessTokenChanged(
				...
				if (currentAccessToken == null) {
					// Bump up the objectSuffix so that we don't have a filename
					// collision between a
					// cache-clear and and a cache-read/write.
					//
					// NOTE: We know that onReceive() was called on the main
					// thread. This means that
					// even this code is running on the main thread, and
					// therefore, there aren't
					// synchronization issues with incrementing the objectSuffix
					// and clearing the
					// caches here.
					...
					appContext
							.getSharedPreferences(LIKE_ACTION_CONTROLLER_STORE,
									Context.MODE_PRIVATE)
							.edit()
							.putInt(LIKE_ACTION_CONTROLLER_STORE_OBJECT_SUFFIX_KEY,
									objectSuffix).apply();

					// Only clearing the actual caches. The MRU index will
					// self-clean with usage.
					// Clearing the caches is necessary to prevent leaking
					// like-state across
					// users.
					cache.clear();
	...
	private static void putControllerInMemoryCache(String objectId,
		...
		mruCacheWorkQueue
				.addActiveWorkItem(new MRUCacheWorkItem(cacheKey, true));

		cache.put(cacheKey, controllerForObject);
	}

	private static LikeActionController getControllerFromInMemoryCache(
			String objectId) {
		String cacheKey = getCacheKeyForObjectId(objectId);
		...
		if (controller != null) {
			...
			mruCacheWorkQueue.addActiveWorkItem(new MRUCacheWorkItem(cacheKey,
					false));
		}
	...
	private static void serializeToDiskAsync(LikeActionController controller) {
		...
		if (!Utility.isNullOrEmpty(controllerJson)
				&& !Utility.isNullOrEmpty(cacheKey)) {
			diskIOWorkQueue.addActiveWorkItem(new SerializeToDiskWorkItem(
					cacheKey, controllerJson));
		}
	}

	/**
	 * NOTE: This MUST be called ONLY via the SerializeToDiskWorkItem class to
	 * ensure that it happens on the right thread, at the right time.
	 */
	private static void serializeToDiskSynchronously(String cacheKey,
	...
	private static LikeActionController deserializeFromDiskSynchronously(
		...
		try {
			...
			if (inputStream != null) {
				String controllerJsonString = Utility
	...
	private static LikeActionController deserializeFromJson(
		...
		try {
			...
			if (version != LIKE_ACTION_CONTROLLER_VERSION) {
				// Don't attempt to deserialize a controller that might be
				// serialized differently
				// than expected.
				return null;
			}

			String objectId = controllerJson
					.getString(JSON_STRING_OBJECT_ID_KEY);
			int objectTypeInt = controllerJson.optInt(JSON_INT_OBJECT_TYPE_KEY,
					LikeView.ObjectType.UNKNOWN.getValue());

			controller = new LikeActionController(objectId,
					LikeView.ObjectType.fromInt(objectTypeInt));

			// Make sure to default to null and not empty string, to keep the
			// logic elsewhere
			// functioning properly.
			controller.likeCountStringWithLike = controllerJson.optString(
					JSON_STRING_LIKE_COUNT_WITH_LIKE_KEY, null);
			controller.likeCountStringWithoutLike = controllerJson.optString(
					JSON_STRING_LIKE_COUNT_WITHOUT_LIKE_KEY, null);
			controller.socialSentenceWithLike = controllerJson.optString(
					JSON_STRING_SOCIAL_SENTENCE_WITH_LIKE_KEY, null);
			controller.socialSentenceWithoutLike = controllerJson.optString(
					JSON_STRING_SOCIAL_SENTENCE_WITHOUT_LIKE_KEY, null);
			controller.isObjectLiked = controllerJson
					.optBoolean(JSON_BOOL_IS_OBJECT_LIKED_KEY);
			controller.unlikeToken = controllerJson.optString(
					JSON_STRING_UNLIKE_TOKEN_KEY, null);

			JSONObject analyticsJSON = controllerJson
					.optJSONObject(JSON_BUNDLE_FACEBOOK_DIALOG_ANALYTICS_BUNDLE);
			if (analyticsJSON != null) {
				controller.facebookDialogAnalyticsBundle = BundleJSONConverter
						.convertToBundle(analyticsJSON);
			}
	...
	private static String serializeToJson(LikeActionController controller) {
		...
		try {
			controllerJson.put(JSON_INT_VERSION_KEY,
					LIKE_ACTION_CONTROLLER_VERSION);
			controllerJson.put(JSON_STRING_OBJECT_ID_KEY, controller.objectId);
			controllerJson.put(JSON_INT_OBJECT_TYPE_KEY,
					controller.objectType.getValue());
			controllerJson.put(JSON_STRING_LIKE_COUNT_WITH_LIKE_KEY,
					controller.likeCountStringWithLike);
			controllerJson.put(JSON_STRING_LIKE_COUNT_WITHOUT_LIKE_KEY,
					controller.likeCountStringWithoutLike);
			controllerJson.put(JSON_STRING_SOCIAL_SENTENCE_WITH_LIKE_KEY,
					controller.socialSentenceWithLike);
			controllerJson.put(JSON_STRING_SOCIAL_SENTENCE_WITHOUT_LIKE_KEY,
					controller.socialSentenceWithoutLike);
			controllerJson.put(JSON_BOOL_IS_OBJECT_LIKED_KEY,
					controller.isObjectLiked);
			controllerJson.put(JSON_STRING_UNLIKE_TOKEN_KEY,
					controller.unlikeToken);
			if (controller.facebookDialogAnalyticsBundle != null) {
				JSONObject analyticsJSON = BundleJSONConverter
						.convertToJSON(controller.facebookDialogAnalyticsBundle);
				if (analyticsJSON != null) {
					controllerJson.put(
	...
	private static String getCacheKeyForObjectId(String objectId) {
		...
		if (accessTokenPortion != null) {
			// Cache-key collisions are not something to worry about here, since
			// we only store state
			// for one access token. Even in the case where the previous access
			// tokens serialized
			// files have not been deleted yet, the objectSuffix will be
			// different due to the access
			// token change, thus making the key different.
			accessTokenPortion = Utility.md5hash(accessTokenPortion);
		}
		return String.format(Locale.ROOT, "%s|%s|com.fb.sdk.like|%d", objectId,
	...
	private static void broadcastAction(LikeActionController controller,
			String action) {
	...
	/**
	 * Gets the String representation of the like-count for the associated
	 * object
	 *
	 * @return String representation of the like-count for the associated object
	 */
	public String getLikeCountString() {
		return isObjectLiked ? likeCountStringWithLike
	...
	public String getSocialSentence() {
		return isObjectLiked ? socialSentenceWithLike
	...
	public boolean shouldEnableView() {
		...
		if (objectIsPage || (objectType == LikeView.ObjectType.PAGE)) {
			...
			// Before any requests are made to the server, we have to rely on
			// the object type set
			// by the app. If we have permissions to make requests, we will know
			// the real type after
			// the first request.
			return false;
		}
		...
		// NOTE: This will NOT be accurate if the app has the type set as
		// UNKNOWN, and the
		// underlying object is a page.
		AccessToken token = AccessToken.getCurrentAccessToken();
		return token != null && token.getPermissions() != null
	 ...
	 */
	public void toggleLike(Activity activity, Fragment fragment,
			Bundle analyticsParameters) {
		getAppEventsLogger().logSdkEvent(
				AnalyticsEvents.EVENT_LIKE_VIEW_DID_TAP, null,
				analyticsParameters);
		...
		if (canUseOGPublish()) {
			...
			if (isPendingLikeOrUnlike) {
				// If the user toggled the button quickly, and there is still a
				// publish underway,
				// don't fire off another request. Also log this behavior.

				getAppEventsLogger().logSdkEvent(
						AnalyticsEvents.EVENT_LIKE_VIEW_DID_UNDO_QUICKLY, null,
						analyticsParameters);
			} else if (!publishLikeOrUnlikeAsync(shouldLikeObject,
					analyticsParameters)) {
				// We were not able to send a graph request to unlike or like
				// the object
			...
			System.out
					.println("presentLikeDialog(activity, fragment, analyticsParameters);");
			presentLikeDialog(activity, fragment, analyticsParameters);
		}
	}
	...
	private AppEventsLogger getAppEventsLogger() {
		if (appEventsLogger == null) {
			appEventsLogger = AppEventsLogger.newLogger(FacebookSdk
					.getApplicationContext());
		}
		return appEventsLogger;
	}

	private boolean publishLikeOrUnlikeAsync(boolean shouldLikeObject,
			Bundle analyticsParameters) {
	...
	/**
	 * Only to be called after an OG-publish was attempted and something went
	 * wrong. The Button state is reverted and an error is returned to the
	 * LikeViews
	 */
	private void publishDidError(boolean oldLikeState) {
		...
		errorBundle.putString(NativeProtocol.STATUS_ERROR_DESCRIPTION,
				ERROR_PUBLISH_ERROR);

		broadcastAction(LikeActionController.this,
				ACTION_LIKE_ACTION_CONTROLLER_DID_ERROR, errorBundle);
	}

	private void updateLikeState(boolean isObjectLiked) {
		updateState(isObjectLiked, this.likeCountStringWithLike,
				this.likeCountStringWithoutLike, this.socialSentenceWithLike,
				this.socialSentenceWithoutLike, this.unlikeToken);
	}

	private void updateState(boolean isObjectLiked,
			String likeCountStringWithLike, String likeCountStringWithoutLike,
			String socialSentenceWithLike, String socialSentenceWithoutLike,
			String unlikeToken) {
		// Normalize all empty strings to null, so that we don't have any
		// problems with comparison.
		likeCountStringWithLike = Utility.coerceValueIfNullOrEmpty(
				likeCountStringWithLike, null);
		likeCountStringWithoutLike = Utility.coerceValueIfNullOrEmpty(
				likeCountStringWithoutLike, null);
		socialSentenceWithLike = Utility.coerceValueIfNullOrEmpty(
				socialSentenceWithLike, null);
		socialSentenceWithoutLike = Utility.coerceValueIfNullOrEmpty(
				socialSentenceWithoutLike, null);
		unlikeToken = Utility.coerceValueIfNullOrEmpty(unlikeToken, null);

		boolean stateChanged = isObjectLiked != this.isObjectLiked
				|| !Utility.areObjectsEqual(likeCountStringWithLike,
						this.likeCountStringWithLike)
				|| !Utility.areObjectsEqual(likeCountStringWithoutLike,
						this.likeCountStringWithoutLike)
				|| !Utility.areObjectsEqual(socialSentenceWithLike,
						this.socialSentenceWithLike)
				|| !Utility.areObjectsEqual(socialSentenceWithoutLike,
	...
	private void presentLikeDialog(final Activity activity,
			final Fragment fragment, final Bundle analyticsParameters) {
		...
		if (analyticsEvent != null) {
			...
			if (fragment != null) {
				...
				System.out
						.println("new LikeDialog(activity).show(likeContent);");
			...
			getAppEventsLogger().logSdkEvent(
					AnalyticsEvents.EVENT_LIKE_VIEW_DID_PRESENT_DIALOG, null,
					analyticsParameters);
		}
	}
	...
	private void onActivityResult(int requestCode, int resultCode, Intent data) {
		// Look for results
		ShareInternalUtility.handleActivityResult(requestCode, resultCode,
				data, getResultProcessor(facebookDialogAnalyticsBundle));

		// The handlers from above will run synchronously. So by the time we get
		// here, it should be
		// safe to stop tracking this call and also serialize the controller to
		// disk
		clearState();
	}
	...
	private ResultProcessor getResultProcessor(final Bundle analyticsParameters) {
		return new ResultProcessor(null) {
			...
			public void onSuccess(AppCall appCall, Bundle data) {
				if (data == null
						|| !data.containsKey(LIKE_DIALOG_RESPONSE_OBJECT_IS_LIKED_KEY)) {
					// This is an empty result that we can't handle.
					return;
				}

				boolean isObjectLiked = data
						.getBoolean(LIKE_DIALOG_RESPONSE_OBJECT_IS_LIKED_KEY);
				...
				if (data.containsKey(LIKE_DIALOG_RESPONSE_LIKE_COUNT_STRING_KEY)) {
					likeCountStringWithLike = data
							.getString(LIKE_DIALOG_RESPONSE_LIKE_COUNT_STRING_KEY);
				...
				if (data.containsKey(LIKE_DIALOG_RESPONSE_SOCIAL_SENTENCE_KEY)) {
					socialSentenceWithLike = data
							.getString(LIKE_DIALOG_RESPONSE_SOCIAL_SENTENCE_KEY);
					socialSentenceWithoutWithoutLike = socialSentenceWithLike;
				}

				String unlikeToken = data
						.containsKey(LIKE_DIALOG_RESPONSE_OBJECT_IS_LIKED_KEY) ? data
						.getString(LIKE_DIALOG_RESPONSE_UNLIKE_TOKEN_KEY)
						: LikeActionController.this.unlikeToken;

				Bundle logParams = (analyticsParameters == null) ? new Bundle()
						: analyticsParameters;
				logParams.putString(AnalyticsEvents.PARAMETER_CALL_ID, appCall
						.getCallId().toString());
				getAppEventsLogger().logSdkEvent(
						AnalyticsEvents.EVENT_LIKE_VIEW_DIALOG_DID_SUCCEED,
						null, logParams);

				updateState(isObjectLiked, likeCountStringWithLike,
						likeCountStringWithoutLike, socialSentenceWithLike,
			...
			public void onError(AppCall appCall, FacebookException error) {
				Logger.log(LoggingBehavior.REQUESTS, TAG,
						"Like Dialog failed with error : %s", error);
	...
	private void saveState(Bundle analyticsParameters) {
		...
		// Serialize to disk, in case we get terminated while waiting for the
		// dialog to complete
		serializeToDiskAsync(this);
	}
	...
	private static void storeObjectIdForPendingController(String objectId) {
		...
		appContext
				.getSharedPreferences(LIKE_ACTION_CONTROLLER_STORE,
						Context.MODE_PRIVATE)
				.edit()
				.putString(LIKE_ACTION_CONTROLLER_STORE_PENDING_OBJECT_ID_KEY,
						objectIdForPendingController).apply();
	}

	private boolean canUseOGPublish() {
		AccessToken accessToken = AccessToken.getCurrentAccessToken();
		// Verify that the object isn't a Page, that we have permissions and
		// that, if we're
		// unliking, then we have an unlike token.
		return !objectIsPage && verifiedObjectId != null && accessToken != null
	...
	private void publishLikeAsync(final Bundle analyticsParameters) {
		...
		fetchVerifiedObjectId(new RequestCompletionCallback() {
			...
			public void onComplete() {
				...
				final PublishLikeRequestWrapper likeRequest = new PublishLikeRequestWrapper(
						verifiedObjectId, objectType);
				...
				requestBatch.addCallback(new GraphRequestBatch.Callback() {
					...
					public void onBatchCompleted(GraphRequestBatch batch) {
						...
						if (likeRequest.error != null) {
							// We already updated the UI to show button in the
							// Liked state. Since
							// this failed, let's revert back to the Unliked
							// state and broadcast
							// an error
							publishDidError(false);
						} else {
							unlikeToken = Utility.coerceValueIfNullOrEmpty(
									likeRequest.unlikeToken, null);
							isObjectLikedOnServer = true;

							getAppEventsLogger().logSdkEvent(
									AnalyticsEvents.EVENT_LIKE_VIEW_DID_LIKE,
									null, analyticsParameters);

							// See if the user toggled the button back while
							// this request was
							// completing
	...
	private void publishUnlikeAsync(final Bundle analyticsParameters) {
		...
		// Perform the Unlike.
		GraphRequestBatch requestBatch = new GraphRequestBatch();
		final PublishUnlikeRequestWrapper unlikeRequest = new PublishUnlikeRequestWrapper(
				unlikeToken);
		...
		requestBatch.addCallback(new GraphRequestBatch.Callback() {
			...
			public void onBatchCompleted(GraphRequestBatch batch) {
				...
				if (unlikeRequest.error != null) {
					// We already updated the UI to show button in the Unliked
					// state. Since this
					// failed, let's revert back to the Liked state and
					// broadcast an error.
					...
					getAppEventsLogger().logSdkEvent(
							AnalyticsEvents.EVENT_LIKE_VIEW_DID_UNLIKE, null,
							analyticsParameters);

					// See if the user toggled the button back while this
					// request was
					// completing
					publishAgainIfNeeded(analyticsParameters);
				}
	...
	private void refreshStatusAsync() {
		AccessToken accessToken = AccessToken.getCurrentAccessToken();
		if (accessToken == null) {
			// Only when we know that there is no active access token should we
			// attempt getting like
			// state from the service. Otherwise, use the access token to make
			// sure we get the
			// correct like state.
		...
		fetchVerifiedObjectId(new RequestCompletionCallback() {
			...
			public void onComplete() {
				final GetOGObjectLikesRequestWrapper objectLikesRequest = new GetOGObjectLikesRequestWrapper(
						verifiedObjectId, objectType);
				final GetEngagementRequestWrapper engagementRequest = new GetEngagementRequestWrapper(
						verifiedObjectId, objectType);
				...
				requestBatch.addCallback(new GraphRequestBatch.Callback() {
					...
					public void onBatchCompleted(GraphRequestBatch batch) {
						if (objectLikesRequest.error != null
							...
							// Refreshing is best-effort. If the refresh fails,
							// don't lose old
							// state.
							Logger.log(
									LoggingBehavior.REQUESTS,
									TAG,
									"Unable to refresh like state for id: '%s'",
									objectId);
							return;
						}
	...
	private void refreshStatusViaService() {
		LikeStatusClient likeStatusClient = new LikeStatusClient(
				...
				FacebookSdk.getApplicationId(), objectId);
		...
		LikeStatusClient.CompletedListener callback = new LikeStatusClient.CompletedListener() {
			...
			public void completed(Bundle result) {
				...
				if (result == null
						|| !result
								.containsKey(ShareConstants.EXTRA_OBJECT_IS_LIKED)) {
					return;
				}

				boolean objectIsLiked = result
						.getBoolean(ShareConstants.EXTRA_OBJECT_IS_LIKED);

				String likeCountWithLike = result
						.containsKey(ShareConstants.EXTRA_LIKE_COUNT_STRING_WITH_LIKE) ? result
						.getString(ShareConstants.EXTRA_LIKE_COUNT_STRING_WITH_LIKE)
						: LikeActionController.this.likeCountStringWithLike;

				String likeCountWithoutLike = result
						.containsKey(ShareConstants.EXTRA_LIKE_COUNT_STRING_WITHOUT_LIKE) ? result
						.getString(ShareConstants.EXTRA_LIKE_COUNT_STRING_WITHOUT_LIKE)
						: LikeActionController.this.likeCountStringWithoutLike;

				String socialSentenceWithLike = result
						.containsKey(ShareConstants.EXTRA_SOCIAL_SENTENCE_WITH_LIKE) ? result
						.getString(ShareConstants.EXTRA_SOCIAL_SENTENCE_WITH_LIKE)
						: LikeActionController.this.socialSentenceWithLike;

				String socialSentenceWithoutLike = result
						.containsKey(ShareConstants.EXTRA_SOCIAL_SENTENCE_WITHOUT_LIKE) ? result
						.getString(ShareConstants.EXTRA_SOCIAL_SENTENCE_WITHOUT_LIKE)
						: LikeActionController.this.socialSentenceWithoutLike;

				String unlikeToken = result
						.containsKey(ShareConstants.EXTRA_UNLIKE_TOKEN) ? result
						.getString(ShareConstants.EXTRA_UNLIKE_TOKEN)
						: LikeActionController.this.unlikeToken;

				updateState(objectIsLiked, likeCountWithLike,
						likeCountWithoutLike, socialSentenceWithLike,
	...
	private void publishAgainIfNeeded(final Bundle analyticsParameters) {
		if (isObjectLiked != isObjectLikedOnServer
			...
			// Unable to re-publish the new desired state. Signal that there is
			// an error and
	...
	private void fetchVerifiedObjectId(
			final RequestCompletionCallback completionHandler) {
		if (!Utility.isNullOrEmpty(verifiedObjectId)) {
		...
		final GetOGObjectIdRequestWrapper objectIdRequest = new GetOGObjectIdRequestWrapper(
				objectId, objectType);
		final GetPageIdRequestWrapper pageIdRequest = new GetPageIdRequestWrapper(
				objectId, objectType);
				...
				if (Utility.isNullOrEmpty(verifiedObjectId)) {
					Logger.log(LoggingBehavior.DEVELOPER_ERRORS, TAG,
							"Unable to verify the FB id for '%s'. Verify that it is a valid FB"
									+ " object or page", objectId);
					logAppEventForError("get_verified_id",
							pageIdRequest.error != null ? pageIdRequest.error
	...
	private void logAppEventForError(String action, Bundle parameters) {
		...
		logParams.putString(AnalyticsEvents.PARAMETER_LIKE_VIEW_OBJECT_ID,
				objectId);
		logParams.putString(AnalyticsEvents.PARAMETER_LIKE_VIEW_OBJECT_TYPE,
				objectType.toString());
		logParams.putString(AnalyticsEvents.PARAMETER_LIKE_VIEW_CURRENT_ACTION,
				action);

		getAppEventsLogger().logSdkEvent(AnalyticsEvents.EVENT_LIKE_VIEW_ERROR,
				null, logParams);
	}
	...
	private class GetOGObjectIdRequestWrapper extends AbstractRequestWrapper {
		...
		GetOGObjectIdRequestWrapper(String objectId,
			...
			setRequest(new GraphRequest(AccessToken.getCurrentAccessToken(),
					"", objectIdRequestParams, HttpMethod.GET));
		}

		@Override
		protected void processError(FacebookRequestError error) {
			// If this object Id is for a Page, an error will be received for
			// this request
		...
		protected void processSuccess(GraphResponse response) {
			JSONObject results = Utility.tryGetJSONObjectFromResponse(
					response.getJSONObject(), objectId);
			if (results != null) {
	...
	private class PublishLikeRequestWrapper extends AbstractRequestWrapper {
		...
		PublishLikeRequestWrapper(String objectId,
			...
			setRequest(new GraphRequest(AccessToken.getCurrentAccessToken(),
					"me/og.likes", likeRequestParams, HttpMethod.POST));
		}

		@Override
		protected void processSuccess(GraphResponse response) {
			unlikeToken = Utility.safeGetStringFromResponse(
		...
		protected void processError(FacebookRequestError error) {
			...
			if (errorCode == ERROR_CODE_OBJECT_ALREADY_LIKED) {
				// This isn't an error for us. Client was just out of sync with
				// server
				// This will prevent us from showing the dialog for this.

				// However, there is no unliketoken. So a subsequent unlike WILL
				// show the dialog
				this.error = null;
			} else {
				Logger.log(LoggingBehavior.REQUESTS, TAG,
	...
	private class PublishUnlikeRequestWrapper extends AbstractRequestWrapper {
		...
		PublishUnlikeRequestWrapper(String unlikeToken) {
			...
			setRequest(new GraphRequest(AccessToken.getCurrentAccessToken(),
					unlikeToken, null, HttpMethod.DELETE));
		}
		...
		protected void processError(FacebookRequestError error) {
			Logger.log(LoggingBehavior.REQUESTS, TAG,
					"Error unliking object with unlike token '%s' : %s",
					unlikeToken, error);
	...
	private class GetOGObjectLikesRequestWrapper extends AbstractRequestWrapper {
		// Initialize the like status to what we currently have. This way,
	...
	private class GetEngagementRequestWrapper extends AbstractRequestWrapper {
		...
		GetEngagementRequestWrapper(String objectId,
			...
			Bundle requestParams = new Bundle();
			requestParams.putString("fields", "engagement.fields("
					+ "count_string_with_like," + "count_string_without_like,"
		...
		@Override
		protected void processSuccess(GraphResponse response) {
			JSONObject engagementResults = Utility
					.tryGetJSONObjectFromResponse(response.getJSONObject(),
							"engagement");
			if (engagementResults != null) {
				// Missing properties in the response should default to cached
				// like status
				likeCountStringWithLike = engagementResults.optString(
						"count_string_with_like", likeCountStringWithLike);

				likeCountStringWithoutLike = engagementResults
						.optString("count_string_without_like",
								likeCountStringWithoutLike);
				...
				socialSentenceStringWithoutLike = engagementResults.optString(
						"social_sentence_without_like",
	...
	private abstract class AbstractRequestWrapper {
		...
		protected void setRequest(GraphRequest request) {
			...
			// Make sure that our requests are hitting the latest version of the
			// API known to this
			// sdk.
	...
	// Performs cache re-ordering/trimming to keep most-recently-used items up
	// front
	// ** NOTE ** It is expected that only _ONE_ MRUCacheWorkItem is ever
	// running. This is enforced
	// by setting the concurrency of the WorkQueue to 1. Changing the
	// concurrency will most likely
	// lead to errors.
	private static class MRUCacheWorkItem implements Runnable {
		...
		public void run() {
			...
			if (shouldTrim && mruCachedItems.size() >= MAX_CACHE_SIZE) {
				int targetSize = MAX_CACHE_SIZE / 2; // Optimize for fewer
														// trim-passes.
				while (targetSize < mruCachedItems.size()) {
					String cacheKey = mruCachedItems.remove(mruCachedItems
							.size() - 1);

					// Here is where we actually remove from the cache of
					// LikeActionControllers.
	...
	private static class CreateLikeActionControllerWorkItem implements Runnable {
		...
		CreateLikeActionControllerWorkItem(String objectId,
				LikeView.ObjectType objectType, CreationCallback callback) {
			this.objectId = objectId;
			this.objectType = objectType;
