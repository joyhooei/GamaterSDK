 ...
 */
public class LikeDialog extends
		FacebookDialogBase<LikeContent, LikeDialog.Result> {

	private static final String TAG = "LikeDialog";

	private static final int DEFAULT_REQUEST_CODE = CallbackManagerImpl.RequestCodeOffset.Like
			.toRequestCode();
	...
	protected void registerCallbackImpl(
		...
		throw new UnsupportedOperationException(
				"registerCallback is not supported for LikeDialog");
	}
			...
			DialogPresenter.setupAppCallForNativeDialog(appCall,
					new DialogPresenter.ParameterProvider() {
						...
						public Bundle getLegacyParameters() {
							// Like is not supported with legacy fb4a devices.
							// Should never get here
							Log.e(TAG,
									"Attempting to present the Like Dialog with an outdated "
	...
	private class WebFallbackHandler extends ModeHandler {
		@Override
		public boolean canShow(final LikeContent content) {
			System.out.println("canShow(final LikeContent content) : "
					+ ((content != null) && LikeDialog.canShowWebFallback()));
	...
	private static Bundle createParameters(final LikeContent likeContent) {
		...
		params.putString(ShareConstants.OBJECT_TYPE, likeContent
