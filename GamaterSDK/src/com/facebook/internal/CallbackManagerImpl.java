...
public final class CallbackManagerImpl implements CallbackManager {
	...
	/**
	 * If there is no explicit callback, but we still need to call the Facebook
	 * component, because it's going to update some state, e.g., login, like.
	 * Then we should register a static callback that can still handle the
	 * response.
	 * 
	 * @param requestCode
	 *            The request code.
	 * @param callback
	 *            The callback for the feature.
	 */
	public synchronized static void registerStaticCallback(int requestCode,
			Callback callback) {
	...
	private static boolean runStaticCallback(int requestCode, int resultCode,
			Intent data) {
		Callback callback = getStaticCallback(requestCode);
		if (Config.isShowLog) {
			LogUtil.printHTTP("Static Callback : " + callback);
		}
		if (callback != null) {
	...
	public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (Config.isShowLog) {
			LogUtil.printHTTP("Callback : " + callback);
		}
		if (callback != null) {
			return callback.onActivityResult(resultCode, data);
		}
	...
	public enum RequestCodeOffset {
		Login(0), Share(1), Message(2), Like(3), GameRequest(4), AppGroupCreate(
				5), AppGroupJoin(6), AppInvite(7), ;
